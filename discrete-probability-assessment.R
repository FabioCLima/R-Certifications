...
nrow(permutations(8,3)) # if orders matters, we must use permutations
                        # nrow will give us the number of the observations

# Question 1b - How many different ways can the three medals be distributed among the 3
# runners from Jamaica?

# solution: Here the orders still matters, because all the 3 runners is from
# Jamaica, so, we should use permutations but only between 3.

nrow(permutations(3,3)) # all the permutations between 3 runners from the same
                        # country

#Question 1c - What is the probability that all 3 medals are won by Jamaica?

# solution:  S = all different ways the 3 medals distributed between 8 runners
# S = nrow(permutations(8,3))
# n(Jamaica runners) = all ways the Jamaica'runners can win
# P_runners_Jamaica = probability that all 3 medals are won by Jamaica
# P_runners_Jamaica = cases favorable to Jamaica's runners/ all cases

runners_Jamaica <- nrow(permutations(3,3)) # Jamaica cases
all_8_runners <- nrow(permutations(8,3))   # all cases
P_runners_Jamaica <- runners_Jamaica / all_8_runners
P_runners_Jamaica

# Question 1d
# Run a Monte Carlo simulation on this vector representing of the 8 runners
# in this race:
...
# we need to create a trial which it will represent the sample from the runners
# vectors, then take at time

# trials
runners_medals <- sample(runners, size = 3)

# Success ( here all winners from Jamaica)
...
(Pr_runners_Jamaica <- mean(medals)) # show all the probabilities for the Jamaica
                                     # runners

# ***************************************************************************
# Question: Restaurante management
# ***************************************************************************
# 
...
#
# Solution : 
# All we need to do here is use counting: 
#1 have only one 1 entree from a list
# of 6 options: nrow(permutations(6,1))
entree <- 6
...
# to calculate all possible combinations for the menu we must multiply the
# combinations above:
...
# Solution:
# For this question we just to update the combinations for drink, instead use
# nrow(permutations(2,1)) --> nrow(permutations(3,1))
...
# Question 2c:
# The manager decides to add the third drink but needs to expand the number
# of options. The manager would prefer not to change his menu further and
...
# The manager isn't sure he can afford to put that many entree choices on 
# the lunch menu and thinks it would be cheaper for him to expand the number
# of sides. He wants to know how many sides he would have to offer to meet 
